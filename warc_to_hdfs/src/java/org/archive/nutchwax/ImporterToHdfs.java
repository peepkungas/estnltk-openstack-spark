 ...
 */
public class ImporterToHdfs extends Configured implements Tool, Mapper<WritableComparable, Writable, Text, NutchWritable>
{
...
/*        
        //test reading
 		Reader seqreader = new Reader(fs, path, conf);
        Text key_r = new Text();
        Text val_r = new Text();
        
        while (seqreader.next(key_r, val_r)) {
            System.out.println(key_r + "\t len: " + val_r.getLength());
        }
*/
   ...
   */
  private boolean importRecord( ARCRecord record, String segmentName, String collectionName, OutputCollector output, Writer writer)
  {
...
//-----------------
  ...
  {
    String usage = 
        "Usage: import_to_hdfs [opts] <manifest> [<segment>]\n" 
      + "Options:\n" 
